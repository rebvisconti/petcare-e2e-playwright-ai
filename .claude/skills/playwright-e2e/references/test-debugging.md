...
## PetCare Common Failures

| Error Pattern | Root Cause | Solution |
|---------------|------------|----------|
| `Target page has been closed` | Server not running | Start backend: `npm start` in petcare-qa repo |
| `Timeout waiting for [data-testid="input-nome-pet"]` | Not authenticated | Check `playwright/.auth/user.json` exists, re-run setup |
| `expected 1, received 2` (appointments count) | Test data not isolated | Add cleanup in `afterEach` or use unique pet names |
| `mensagem-global not visible` | Form validation failed | Check browser console for validation errors |
| Login redirects to `/login.html` | Auth state expired | Delete `playwright/.auth/user.json` and re-run setup |
...
## PetCare Environment Variables

### Required
- `BASE_URL`: Application URL (default: `http://localhost:3001`)

### Optional
- `HEADLESS`: Run tests in headless mode (default: `false` in dev, `true` in CI)
- `SLOWMO`: Slow down Playwright operations in ms (default: `0`)
...
**Note:** PetCare currently runs only locally. No staging/production environments configured yet.
